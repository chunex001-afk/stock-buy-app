...
def fetch_price(ticker, force=False):
    ...
    try:
        ...
        # Recent high / overheating. Near a high is not automatically bad when
        # the trend is strong. A confirmed trend gets a smaller penalty and a
        # pullback from the high can become a better entry setup.
        if high_gap > -3:
            score -= 1 if strong_trend else 4
        elif high_gap <= -3 and high_gap >= -12 and strong_trend:
            score += 3  # healthy pullback inside an established uptrend
        elif high_gap < -15:
            score += 2
...
def reason_for(ticker, data, news):
    ...
    if strong_trend and rsi is not None and rsi > 65:
        if high_gap <= -3:
            return "強い上昇トレンドを維持。やや過熱気味だが、押し目では買いやすい"
        return "業績等の材料確認と合わせて、強い上昇トレンドを維持。短期的な過熱には注意"
    if month >= 15:
...
株価モメンタム、MA20/MA50、RSI14、直近高値からの距離、出来高を組み合わせて0〜100で算出。RSIだけで売買判断はしません。強い上昇トレンド中の高RSIは過度に減点せず、「強いがやや過熱・押し目で買いやすい」と評価します。<br>
