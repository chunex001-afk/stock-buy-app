...
def fetch_price(ticker, force=False):
    ...
    try:
        ...
        # RSI = buying opportunity, not a standalone buy/sell rule.
        if rsi is not None:
            if 45 <= rsi <= 65:
                score += 6
            elif 65 < rsi <= 72:
...
def reason_for(ticker, data, news):
    ...
    if data.get("month_return", 0) >= 15:
        return "1か月上昇率が高く、上昇モメンタムが強い"
    if data.get("rsi14") is not None and data["rsi14"] > 72:
        return "RSIが高く、短期的な過熱に注意"
    if data.get("rsi14") is not None and data["rsi14"] < 35:
...
株価モメンタム、MA20/MA50、RSI14、直近高値からの距離、出来高を組み合わせて0〜100で算出。RSIだけで売買判断はしません。<br>
